...
Options:
    ...
    --set=[]:
	overrides templated manifest fields by provided key-value pairs; on `exec`, entries are also injected as environment variables into custom action containers

    --set-value-file='':
	overrides templated manifest fields by a file containing key-value pairs in .env file format; on `exec`, entries are also injected as environment variables into custom action containers
